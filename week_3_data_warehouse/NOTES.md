...
MOST NOTES COPIED FROM [LINK](https://github.com/ziritrion/dataeng-zoomcamp/blob/main/notes/3_data_warehouse.md#data-warehouse)
## OLAP vs OLTP

OLAP: Online Transaction Processing

- used for a lot of data and finding hidden insights
- big updates

OLTP: Online Analytical Processing

- used for backend services where you want to group a couple of SQL queries and fallback if one fails
- fast and short updates
...
- OLAP Solution
- Used for reportina and data analysis

![image](https://github.com/ziritrion/dataeng-zoomcamp/blob/main/notes/images/03_01.jpeg?raw=true)

## Big Query

- Data warehouse solution **SERVERLESS**
  - Software + Infrastructure is provided
- Built in features
- Maximizes flexibility by separating compute that analyzes data from storage
...
-- Create an external table referring to gcs path
  -- We are going to get data from the 'external' google cloud storage
CREATE OR REPLACE EXTERNAL TABLE `dezoomcamp.external_yellow_tripdata`
OPTIONS (
  format = 'parquet',
  uris = ['gs://dtc_data_lake_dezoomcamp2023/data/yellow/yellow_tripdata_2019-*.parquet', 'gs://dtc_data_lake_dezoomcamp2023/data/yellow/yellow_tripdata_2020-*.parquet']
);
```

Dataset needs to be in the same location (geographically) as the GCS bucket.
...
```sql
CREATE OR REPLACE TABLE dezoomcamp2.yellow_tripdata_partitoned
PARTITION BY
  DATE(tpep_pickup_datetime) AS
SELECT * FROM dezoomcamp2.external_yellow_tripdata;
```
...
FROM dezoomcamp2.yellow_tripdata_non_partitoned
...
```sql
SELECT DISTINCT(VendorID)
FROM dezoomcamp2.yellow_tripdata_partitoned
WHERE DATE(tpep_pickup_datetime) BETWEEN '2019-06-01' AND '2019-06-30';
```
...
Seems like it may be able to be called 'pre-sorting' for each partition
...
```sql
CREATE OR REPLACE TABLE dezoomcamp2.yellow_tripdata_partitoned_clustered
PARTITION BY DATE(tpep_pickup_datetime)
CLUSTER BY VendorID AS
SELECT * FROM dezoomcamp2.external_yellow_tripdata;
```

^^ This query is good if you are repeatedly querying by the date and vendor
...
```sql
SELECT count(*) as trips
FROM dezoomcamp2.yellow_tripdata_partitoned_clustered
WHERE DATE(tpep_pickup_datetime) BETWEEN '2019-06-01' AND '2020-12-31'
  AND VendorID=1;
```
